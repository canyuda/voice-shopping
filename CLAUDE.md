...
- Flyway 已执行的迁移脚本（`db/migration/V{n}__*.sql`）禁止修改，修改会导致 checksum 校验失败。如需变更，新建下一个版本号的迁移脚本。修改前必须确认该脚本是否已执行。
- 优先使用函数式编程范式
- 错误处理遵循 fail fast 原则，严禁吞掉异常
- 禁止硬编码（中间件配置、第三方接口地址等走配置文件）
- 简洁优于巧妙，可读性第一
- 接口请求体和响应体禁止使用 `Map<String, Object>`，必须定义专用类（Record 或 POJO），避免魔法值
