...
订单查询能力（用户视角 + 商家视角）。所有订单查询路径**强制带主体维度**（`userId` 或 `merchantId`），通过 Repository 派生查询命名（`findByIdAndUserId` / `findAllByMerchantIdOrderBy...`）让"忘传身份"的越权写法在编译期就被拦下，而不是依赖"先 `findById` 再判 ownership"的事后 if 校验——后者一旦遗漏直接越权，且 403/404 的差异会泄露资源存在性。

本能力是 CLAUDE.md「主体强制过滤原则」章节的首个落点；后续 `refund_record` / 用户消息历史等同类私有数据实体复用相同模式。
...
- `PageInfo<OrderDTO> listMine(Long userId, Pageable pageable)`：调用 `orderRecordRepository.findAllByUserIdOrderByCreatedAtDesc(userId, pageable)`，将 `Page<OrderRecord>` 适配为 `PageInfo<OrderDTO>` 返回（屏蔽 Spring Data `Page` 抽象，仅暴露精简分页字段，详见 PageInfo 响应包装 Requirement）。
- `PageInfo<OrderDTO> listForMerchant(Long merchantId, Pageable pageable)`：直接调用 `orderRecordRepository.findAllByMerchantIdOrderByCreatedAtDesc(merchantId, pageable)`，**不做角色校验**（当前阶段调用方为可信内部 Service / 后台任务）。

`OrderService` MUST NOT 依赖 Sa-Token / `StpUtil`；`userId` 与 `merchantId` 一律由调用方（Controller / 定时任务 / 测试）显式传入，与既有 `SessionService` 等惯例一致。`voice-shopping-business/pom.xml` MUST NOT 引入 sa-token 依赖。
...
- **THEN** 返回的 `PageInfo<OrderDTO>` 中 `list` 全部条目 `userId == 7`，按 `createdAt` 降序
...
- `GET /api/v1/orders/mine`：接收 Spring 自动绑定的 `Pageable`（默认 `page=0`、`size=20`、`sort=createdAt,desc` 由 Service 兜底），返回 `ApiResult<PageInfo<OrderDTO>>`，Controller 内通过 `StpUtil.getLoginIdAsLong()` 取当前 userId，调用 `orderService.listMine(userId, pageable)`。
...
Controller MUST 通过 `StpUtil.getLoginIdAsLong()` 取当前登录 userId（Sa-Token 调用仅在 Controller 层发生，业务层不感知 Sa-Token）；MUST NOT 自行从 Header 解析 token（Sa-Token 拦截器/过滤器已处理）；MUST NOT 自行构建错误响应（异常交给 `GlobalExceptionHandler`）。
...
- **THEN** Sa-Token 通过校验，Controller 通过 `StpUtil.getLoginIdAsLong()` 取得 userId=7，调用 `orderService.listMine(7L, pageable)`，响应 `ApiResult.ok(pageInfo)`，HTTP 200
...
由于 `OrderDTO` 在 `voice-shopping-common` 模块、不能反向依赖 `voice-shopping-infrastructure`，entity → DTO 的映射 SHALL 放在 `OrderService` 内（私有方法 `toDto(OrderRecord)`），并在分页路径上通过私有 `toPageInfo(Page<OrderRecord>)` 一次性完成 entity Page → `PageInfo<OrderDTO>` 的转换；`items` / `aiContext` 不为 null 时直接透传，为 null 时按 `Collections.emptyList()` / `Collections.emptyMap()` 兜底。`items` 中每个 `OrderItemJson` 同样通过 lambda 映射为 `OrderItemDTO`。
...
### Requirement: PageInfo 响应包装

系统 SHALL 提供 `PageInfo<T>` Java Record，位置为 `com.voiceshopping.common.dto.PageInfo`，作为分页接口对外的响应包装，**取代** Spring Data `Page` 直接序列化（`Page` 默认序列化会带出 `pageable.sort.unsorted` / `pageable.unpaged` / `numberOfElements` / `empty` 等噪声字段，客户端不需要）。

字段 SHALL 仅包含五个：`list` (`List<T>`) / `page` (int，0-based，与请求 `?page=` 一致) / `size` (int) / `total` (long) / `totalPages` (int)。

`PageInfo` MUST NOT 依赖 Spring Data 任何类（`voice-shopping-common` 保持依赖最小化）。`Page<Entity>` → `PageInfo<DTO>` 的适配 SHALL 在 Service 层完成（如 `OrderService#toPageInfo`），Service 方法签名直接返回 `PageInfo<T>`，不向 Controller 泄露 `Page` 抽象。

#### Scenario: PageInfo 字段精简
- **WHEN** Controller 返回 `ApiResult.ok(orderService.listMine(userId, PageRequest.of(0, 20)))`，Jackson 序列化响应体
- **THEN** `data` 仅包含 `list` / `page` / `size` / `total` / `totalPages` 五个字段，无 `pageable` / `sort` / `empty` / `numberOfElements` 等

#### Scenario: 0-based 页码与请求一致
- **WHEN** 客户端请求 `GET /api/v1/orders/mine?page=2&size=10`
- **THEN** 响应 `data.page == 2`、`data.size == 10`，无需任何 +1/-1 转换
