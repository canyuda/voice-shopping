...
多商家数据隔离能力：在 AppUser/CurrentUser 身份基础上，引入 Channel 与 SessionScope 概念，通过 Sa-Token sub-protocol token 鉴权 + Redis 旁路缓存的 SessionScopeCache 把会话锁定到一个或多个商家 / 商品，并在向量召回链路（RecommendOrchestrator / ParallelRecommendService / SearchController 带 sessionId 时）通过 ScopeFilterBuilder 强制叠加 `merchant_id IN (...)` 过滤，缓存 miss 时降级为全平台并 WARN 日志。
